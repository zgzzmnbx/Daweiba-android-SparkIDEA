...
- 2026-08-04：完成“大尾巴闪念”待办提醒 P0 v0.48，新增提醒协议兼容、独立 SQLite `reminders` 表、单次 AlarmManager 调度、通知动作、重启/时间变化恢复、逾期不补发、手机端添加/修改/取消提醒，以及 Obsidian 插件 `v1.3.0` 的稳定任务 ID 与提醒字段输出。已通过插件测试、纯 Java 测试、资源检查、APK 构建和签名校验；本次 `adb devices` 未发现 vivo X200 Pro，真机相关场景待设备连接后补验。
- 2026-08-04：完成“大尾巴闪念”待办提醒 P1 v0.49，新增每日待办概览、低打扰通知通道、可选低频后台 WebDAV 同步、自然语言提醒确认、多级提醒、提醒诊断和锁屏内容隐藏；已通过 P0/P1 纯 Java 测试、资源检查、APK 构建、Manifest 反查和签名校验。本次 `adb devices` 仍未发现在线设备，真机安装及锁屏、重启、Doze、后台同步现场验证待设备连接后补验。
