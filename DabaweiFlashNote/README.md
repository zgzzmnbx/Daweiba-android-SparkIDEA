...
- Rhea UI（2026-08-07）：已按 preset `b1au7YYAi` 完成原生 XML + Java 的 Rhea / Neutral / Blue 双主题重绘：语义令牌、浅/深资源、`跟随系统 / 浅色 / 深色` 偏好、八个旧主题迁移、首页/历史/待办/设置/快速捕捉/弹窗/桌面组件资源和敏感字段遮罩均已接入；未改变 SQLite、WebDAV、提醒、飞书、分享或图片链路。五项既有回归、新 UI 契约、故障注入红→绿、构建签名均已通过。vivo 真机两次安装均被系统返回 `INSTALL_FAILED_ABORTED: User rejected permissions`，16 张现场截图待 USB 安装授权后补做，详见上级 `BLOCKED.md`。
...
- 当前交付状态：`0.6.0-shadcn-rhea-ui` 代码和本地 APK 已完成；真机安装授权与浅/深主题现场截图仍被设备系统拦截，不能把本地构建通过写成视觉验收完成。详见 `PRD.md` 7.6 节、`../BLOCKED.md` 和 `../PROGRESS.md`。
