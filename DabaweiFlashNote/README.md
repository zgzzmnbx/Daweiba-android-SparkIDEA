...
- 当前构建目标：`versionCode=53`，`versionName=0.7.0-cloud-feishu-relay`；云端飞书提醒中继已部署为独立 HTTPS 服务，v52 UI 基线保持不变。
...
- v0.7.0 新增云端飞书提醒中继：手机读取 WebDAV 并上报提醒快照，云主机不读取 WebDAV，只负责持久化、到点发送和失败重试；设置页开关默认打开。开关打开后手机不再直推飞书，关闭后恢复原有手机直推。
